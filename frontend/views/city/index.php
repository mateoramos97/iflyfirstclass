...
$this->title = $city_model->browser_title;
$this->registerMetaTag(['property' => 'og:locale', 'content' => 'en']);
$this->registerMetaTag(['property' => 'og:title', 'content' => $city_model->browser_title]);
$this->registerMetaTag(['property' => 'og:description', 'content' => $city_model->description]);
...
$this->params['breadcrumbs'][] = Html::encode($city_model['name']);
?>
				...
				<div class="content flex flex-col align-center xl:p-12 p-6 xl:col-span-5 col-span-11 justify-between h-full">
					...
					<div class="xl:text-left text-center">
						...
						<h1 class="title-form font-silk-serif-medium text-white 2xl:text-8xl xl:text-7xl md:text-2xl mt-3">
							<span itemprop="name"><?= Html::encode($city_model['name']) ?></span>
						</h1>
					...
					<div class="mt-16 xl:block hidden">
						...
						<div class="flex items-center bg-black p-2 w-fit">
							<img class="mr-2 scale-75" src="/public/img/phone-operator.svg" alt="" width="40" height="40">
    ...
    <div class="highlight-block-wrapper pt-20 pb-8 bg-secondary -mt-10">
		<div class="container mx-auto xl:px-12 px-4">
			<div class="title text-2xl mt-2 font-gilroy-regular">
				<span>Special offer to</span> <span class="font-semibold"><?= Html::encode($city_model['name']) ?></span>
			</div>
						...
						<div class="flex justify-between mt-8">
							<p itemprop="offers" itemscope="" itemtype="http://schema.org/Offer" class="price-new">
								...
								<span class="font-gilroy-semibold text-orange text-lg">
									<span>$</span><span itemprop="price"><?= Html::encode($city_model['business_class_price']) ?></span>
								</span>
							</p>
							<p class="price-old">
								Old price <span class="line-through">$<?= Html::encode($city_model['business_class_old_price']) ?></span>
							</p>
						</div>
					</div>
				</div>
					...
					<div class="col-span-5 px-10 py-8">
						...
						<div class="flex justify-between mt-8">
							<p itemprop="offers" itemscope="" itemtype="http://schema.org/Offer" class="price-new">
								...
								Now:
								<span class="font-gilroy-semibold text-orange text-lg">
									<span>$</span><span itemprop="price"><?= Html::encode($city_model['first_class_price']) ?></span>
								</span>
							</p>
							<p class="price-old">
								Old price <span class="line-through">$<?= Html::encode($city_model['first_class_old_price']) ?></span>
							</p>
						</div>
					</div>
				</div>
	...
	<div class="page-content xl:my-20 my-10">
		<div class="container mx-auto xl:px-10 px-2 grid grid-cols-12 xl:gap-5 gap-1 box-border">
			<div class="content xl:col-span-9 col-span-12 xl:pr-16 pr-0">
				<h3>
					<?= Html::encode($city_model['sub_title']) ?>
				</h3>
				<p class="text-gray-2 my-12 font-proxima-nova-light"><?= $city_model->summary ?></p>
				<div class="body">
					<?php echo $city_model->body_column_1; ?>
					<?php echo $city_model->body_column_2; ?>
                </div>
				<div class="mt-10 xl:p-8 p-4 text-gray-2 flex flex-wrap gap-2 rounded-2xl border border-gray-light-2 items-start xl:justify-between justify-center">
					<p class="xl:w-3/4 text-justify">IFlyFirstClass offers cheap Business Class flights to <?= Html::encode($city_model['name']) ?>, save thousands on last minute Business Class tickets. Best deals on Business Class. Special fares on Business and First Class tickets. Discounted First & Business Class airline tickets. First Class & Business Class travel deals.</p>
